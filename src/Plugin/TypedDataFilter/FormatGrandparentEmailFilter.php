...
/**
 * A data filter which gets the email address of a comment's grandparent,
 * if there is one, or an empty string otherwise.
 *
 * @DataFilter(
 *   id = "comment_grandparent_email",
 *   label = @Translation("Get the email address of a comment's grandparent."),
 * )
 */
class FormatGrandparentEmailFilter extends DataFilterBase {
    ...
    public function filter(DataDefinitionInterface $definition, $value,
        ...
        if ($value->hasParentComment()) {
            ...
            if ($pc->hasParentComment()) {
                $gpc = $pc->getParentComment();
                $gpae = $gpc->getAuthorEmail();
                return $gpae;
            }
        }
        // No grandparent comment.
        return '';
  }
